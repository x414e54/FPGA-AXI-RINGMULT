//===-- HEIIS.h - Top-level interface for HEIIS representation --*- C++ -*-===//
//
...
namespace llvm {
  class FunctionPass;
  class HEIISTargetMachine;
  class formatted_raw_ostream;
  class AsmPrinter;
  class MCInst;
  class MachineInstr;

  FunctionPass *createHEIISISelDag(HEIISTargetMachine &TM);
  FunctionPass *createHEIISDelaySlotFillerPass(TargetMachine &TM);

  void LowerHEIISMachineInstrToMCInst(const MachineInstr *MI,
                                      MCInst &OutMI,
                                      AsmPrinter &AP);
} // end namespace llvm;

namespace llvm {
  // Enums corresponding to HEIIS condition codes, both icc's and fcc's.  These
