//===-- HEIISSubtarget.h - Define Subtarget for the SPARC -------*- C++ -*-===//
//
...
class HEIISSubtarget : public HEIISGenSubtargetInfo {
  Triple TargetTriple;
  virtual void anchor();
  bool IsV9;
  bool IsLeon;
  ...
  HEIISInstrInfo InstrInfo;
  HEIISTargetLowering TLInfo;
  SelectionDAGTargetInfo TSInfo;
  HEIISFrameLowering FrameLowering;

public:
  HEIISSubtarget(const Triple &TT, const std::string &CPU,
                 const std::string &FS, const TargetMachine &TM, bool is64bit);

  const HEIISInstrInfo *getInstrInfo() const override { return &InstrInfo; }
  const TargetFrameLowering *getFrameLowering() const override {
    return &FrameLowering;
  }
  const HEIISRegisterInfo *getRegisterInfo() const override {
    return &InstrInfo.getRegisterInfo();
  }
  const HEIISTargetLowering *getTargetLowering() const override {
    return &TLInfo;
  }
  ...
  HEIISSubtarget &initializeSubtargetDependencies(StringRef CPU, StringRef FS);
