//===-- HEIISISelLowering.cpp - HEIIS DAG Lowering Implementation ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the interfaces that HEIIS uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//

#include "HEIISISelLowering.h"
...
static bool CC_HEIIS_Assign_SRet(unsigned &ValNo, MVT &ValVT,
...
static bool CC_HEIIS_Assign_Split_64(unsigned &ValNo, MVT &ValVT,
                                     MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                     ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
...
static bool CC_HEIIS_Assign_Ret_Split_64(unsigned &ValNo, MVT &ValVT,
                                         MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                         ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
  static const MCPhysReg RegList[] = {
...
static bool CC_HEIIS64_Full(unsigned &ValNo, MVT &ValVT,
                            MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
static bool CC_HEIIS64_Half(unsigned &ValNo, MVT &ValVT,
                            MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
#include "HEIISGenCallingConv.inc"

// The calling conventions in HEIISCallingConv.td are described in terms of the
...
SDValue
HEIISTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                 bool IsVarArg,
...
SDValue
HEIISTargetLowering::LowerReturn_32(SDValue Chain, CallingConv::ID CallConv,
                                    bool IsVarArg,
                                    const SmallVectorImpl<ISD::OutputArg> &Outs,
                                    const SmallVectorImpl<SDValue> &OutVals,
                                    const SDLoc &DL, SelectionDAG &DAG) const {
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_HEIIS32);
...
SDValue
HEIISTargetLowering::LowerReturn_64(SDValue Chain, CallingConv::ID CallConv,
                                    bool IsVarArg,
                                    const SmallVectorImpl<ISD::OutputArg> &Outs,
                                    const SmallVectorImpl<SDValue> &OutVals,
                                    const SDLoc &DL, SelectionDAG &DAG) const {
  ...
  // Analyze return values.
  CCInfo.AnalyzeReturn(Outs, RetCC_HEIIS64);
...
SDValue HEIISTargetLowering::LowerFormalArguments(
...
SDValue HEIISTargetLowering::LowerFormalArguments_32(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineRegisterInfo &RegInfo = MF.getRegInfo();
  HEIISMachineFunctionInfo *FuncInfo = MF.getInfo<HEIISMachineFunctionInfo>();

  // Assign locations to all of the incoming arguments.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_HEIIS32);
...
SDValue HEIISTargetLowering::LowerFormalArguments_64(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  MachineFunction &MF = DAG.getMachineFunction();

  // Analyze arguments according to CC_HEIIS64.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_HEIIS64);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    assert(VA.isMemLoc());
    // The CC_HEIIS64_Full/Half functions compute stack offsets relative to the
...
SDValue
HEIISTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
...
SDValue
HEIISTargetLowering::LowerCall_32(TargetLowering::CallLoweringInfo &CLI,
                                  SmallVectorImpl<SDValue> &InVals) const {
  ...
  // HEIIS target does not yet support tail call optimization.
  isTailCall = false;
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_HEIIS32);
  ...
  unsigned TF = isPositionIndependent() ? HEIISMCExpr::VK_HEIIS_WPLT30 : 0;
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee))
  ...
  const HEIISRegisterInfo *TRI = Subtarget->getRegisterInfo();
  const uint32_t *Mask =
      ((hasReturnsTwice)
  ...
  RVInfo.AnalyzeCallResult(Ins, RetCC_HEIIS32);
...
unsigned
HEIISTargetLowering::getSRetArgSize(SelectionDAG &DAG, SDValue Callee) const
{
  const Function *CalleeFn = nullptr;
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
    CalleeFn = dyn_cast<Function>(G->getGlobal());
...
SDValue
HEIISTargetLowering::LowerCall_64(TargetLowering::CallLoweringInfo &CLI,
                                  SmallVectorImpl<SDValue> &InVals) const {
  SelectionDAG &DAG = CLI.DAG;
  SDLoc DL = CLI.DL;
  SDValue Chain = CLI.Chain;
  ...
  CCState CCInfo(CLI.CallConv, CLI.IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(CLI.Outs, CC_HEIIS64);

  // Get the size of the outgoing arguments stack space requirement.
  // The stack offset computed by CC_HEIIS64 includes all arguments.
  ...
  RVInfo.AnalyzeCallResult(CLI.Ins, RetCC_HEIIS64);
...
TargetLowering::AtomicExpansionKind HEIISTargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *AI) const {
...
HEIISTargetLowering::HEIISTargetLowering(const TargetMachine &TM,
                                         const HEIISSubtarget &STI)
  ...
  // HEIIS doesn't have i1 sign extending load
  ...
  // HEIIS doesn't have sext_inreg, replace them with shl/sra
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i16, Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i8 , Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i1 , Expand);

  // HEIIS has no REM or DIVREM operations.
  ...
  // HEIIS has no select or setcc: expand to SELECT_CC.
  ...
  // HEIIS doesn't have BRCOND either, it has BR_CC.
  ...
  // ATOMICs.
  // Atomics are supported on HEIISV9. 32-bit atomics are also
  // supported by some Leon HEIISV8 variants. Otherwise, atomics
  // are unsupported.
  ...
  if (!Subtarget->isV9()) {
    // HEIISV8 does not have FNEGD and FABSD.
  ...
  // FIXME: HEIIS provides these multiplies, but we don't have them yet.
...
bool HEIISTargetLowering::useSoftFloat() const {
  return Subtarget->useSoftFloat();
}

const char *HEIISTargetLowering::getTargetNodeName(unsigned Opcode) const {
...
EVT HEIISTargetLowering::getSetCCResultType(const DataLayout &, LLVMContext &,
                                            EVT VT) const {
  if (!VT.isVector())
    return MVT::i32;
...
/// combiner.
void HEIISTargetLowering::computeKnownBitsForTargetNode
                                (const SDValue Op,
                                 APInt &KnownZero,
                                 APInt &KnownOne,
...
SDValue HEIISTargetLowering::withTargetFlags(SDValue Op, unsigned TF,
                                             SelectionDAG &DAG) const {
...
SDValue HEIISTargetLowering::makeHiLoPair(SDValue Op,
...
SDValue HEIISTargetLowering::makeAddress(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (isPositionIndependent()) {
    ...
    SDValue HiLo = makeHiLoPair(Op, HEIISMCExpr::VK_HEIIS_GOT22,
                                HEIISMCExpr::VK_HEIIS_GOT10, DAG);
  ...
  case CodeModel::Small:
    // abs32.
    return makeHiLoPair(Op, HEIISMCExpr::VK_HEIIS_HI,
                        HEIISMCExpr::VK_HEIIS_LO, DAG);
  case CodeModel::Medium: {
    // abs44.
    SDValue H44 = makeHiLoPair(Op, HEIISMCExpr::VK_HEIIS_H44,
                               HEIISMCExpr::VK_HEIIS_M44, DAG);
    H44 = DAG.getNode(ISD::SHL, DL, VT, H44, DAG.getConstant(12, DL, MVT::i32));
    SDValue L44 = withTargetFlags(Op, HEIISMCExpr::VK_HEIIS_L44, DAG);
    L44 = DAG.getNode(SPISD::Lo, DL, VT, L44);
    return DAG.getNode(ISD::ADD, DL, VT, H44, L44);
  }
  case CodeModel::Large: {
    // abs64.
    SDValue Hi = makeHiLoPair(Op, HEIISMCExpr::VK_HEIIS_HH,
                              HEIISMCExpr::VK_HEIIS_HM, DAG);
    Hi = DAG.getNode(ISD::SHL, DL, VT, Hi, DAG.getConstant(32, DL, MVT::i32));
    SDValue Lo = makeHiLoPair(Op, HEIISMCExpr::VK_HEIIS_HI,
                              HEIISMCExpr::VK_HEIIS_LO, DAG);
    return DAG.getNode(ISD::ADD, DL, VT, Hi, Lo);
  }
  }
...
SDValue HEIISTargetLowering::LowerGlobalAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue HEIISTargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue HEIISTargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue HEIISTargetLowering::LowerGlobalTLSAddress(SDValue Op,
                                                   SelectionDAG &DAG) const {
  ...
  if (model == TLSModel::GeneralDynamic || model == TLSModel::LocalDynamic) {
    unsigned HiTF = ((model == TLSModel::GeneralDynamic)
                     ? HEIISMCExpr::VK_HEIIS_TLS_GD_HI22
                     : HEIISMCExpr::VK_HEIIS_TLS_LDM_HI22);
    unsigned LoTF = ((model == TLSModel::GeneralDynamic)
                     ? HEIISMCExpr::VK_HEIIS_TLS_GD_LO10
                     : HEIISMCExpr::VK_HEIIS_TLS_LDM_LO10);
    unsigned addTF = ((model == TLSModel::GeneralDynamic)
                      ? HEIISMCExpr::VK_HEIIS_TLS_GD_ADD
                      : HEIISMCExpr::VK_HEIIS_TLS_LDM_ADD);
    unsigned callTF = ((model == TLSModel::GeneralDynamic)
                       ? HEIISMCExpr::VK_HEIIS_TLS_GD_CALL
                       : HEIISMCExpr::VK_HEIIS_TLS_LDM_CALL);
    ...
    SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                 withTargetFlags(Op, HEIISMCExpr::VK_HEIIS_TLS_LDO_HIX22, DAG));
    SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                 withTargetFlags(Op, HEIISMCExpr::VK_HEIIS_TLS_LDO_LOX10, DAG));
    HiLo =  DAG.getNode(ISD::XOR, DL, PtrVT, Hi, Lo);
    return DAG.getNode(SPISD::TLS_ADD, DL, PtrVT, Ret, HiLo,
                   withTargetFlags(Op, HEIISMCExpr::VK_HEIIS_TLS_LDO_ADD, DAG));
  }

  if (model == TLSModel::InitialExec) {
    unsigned ldTF     = ((PtrVT == MVT::i64)? HEIISMCExpr::VK_HEIIS_TLS_IE_LDX
                         : HEIISMCExpr::VK_HEIIS_TLS_IE_LD);
    ...
    SDValue TGA = makeHiLoPair(Op,
                               HEIISMCExpr::VK_HEIIS_TLS_IE_HI22,
                               HEIISMCExpr::VK_HEIIS_TLS_IE_LO10, DAG);
  ...
  SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                  withTargetFlags(Op, HEIISMCExpr::VK_HEIIS_TLS_LE_HIX22, DAG));
  SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                  withTargetFlags(Op, HEIISMCExpr::VK_HEIIS_TLS_LE_LOX10, DAG));
...
SDValue
HEIISTargetLowering::LowerF128Op(SDValue Op, SelectionDAG &DAG,
...
SDValue HEIISTargetLowering::LowerF128Compare(SDValue LHS, SDValue RHS,
...
SDValue HEIISTargetLowering::LowerEH_SJLJ_SETJMP(SDValue Op, SelectionDAG &DAG,
    const HEIISTargetLowering &TLI) const {
  SDLoc DL(Op);
...
SDValue HEIISTargetLowering::LowerEH_SJLJ_LONGJMP(SDValue Op, SelectionDAG &DAG,
    const HEIISTargetLowering &TLI) const {
  SDLoc DL(Op);
...
static SDValue LowerDYNAMIC_STACKALLOC(SDValue Op, SelectionDAG &DAG,
                                       const HEIISSubtarget *Subtarget) {
...
SDValue HEIISTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                     SelectionDAG &DAG) const {
...
MachineBasicBlock *
HEIISTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
...
MachineBasicBlock *
HEIISTargetLowering::expandSelectCC(MachineInstr &MI, MachineBasicBlock *BB,
...
MachineBasicBlock *
HEIISTargetLowering::emitEHSjLjLongJmp(MachineInstr &MI,
...
MachineBasicBlock *
HEIISTargetLowering::emitEHSjLjSetJmp(MachineInstr &MI,
                                      MachineBasicBlock *MBB) const {
  DebugLoc DL = MI.getDebugLoc();
  const TargetInstrInfo *TII = Subtarget->getInstrInfo();
...
//===----------------------------------------------------------------------===//
//                         HEIIS Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
HEIISTargetLowering::ConstraintType
HEIISTargetLowering::getConstraintType(StringRef Constraint) const {
...
TargetLowering::ConstraintWeight HEIISTargetLowering::
...
HEIISTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
...
bool
HEIISTargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  // The HEIIS target isn't yet aware of offsets.
  return false;
}

void HEIISTargetLowering::ReplaceNodeResults(SDNode *N,
...
bool HEIISTargetLowering::useLoadStackGuardNode() const {
...
void HEIISTargetLowering::insertSSPDeclarations(Module &M) const {
  if (!Subtarget->isTargetLinux())
