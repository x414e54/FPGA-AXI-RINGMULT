...
static cl::opt<bool> DisableDelaySlotFiller(
  ...
  cl::init(false),
  cl::desc("Disable the HEIIS delay slot filler."),
  cl::Hidden);
...
namespace {
  struct Filler : public MachineFunctionPass {
    ...
    bool runOnMachineFunction(MachineFunction &F) override {
      bool Changed = false;
      Subtarget = &F.getSubtarget<HEIISSubtarget>();
...
/// createHEIISDelaySlotFillerPass - Returns a pass that fills in delay
/// slots in HEIIS MachineFunctions
///
FunctionPass *llvm::createHEIISDelaySlotFillerPass(TargetMachine &tm) {
  return new Filler;
}
...
bool Filler::runOnMachineBasicBlock(MachineBasicBlock &MBB) {
  bool Changed = false;
  Subtarget = &MBB.getParent()->getSubtarget<HEIISSubtarget>();
