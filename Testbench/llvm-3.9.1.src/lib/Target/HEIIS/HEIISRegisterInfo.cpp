//===-- HEIISRegisterInfo.cpp - SPARC Register Information ----------------===//
//
...
const uint32_t *
HEIISRegisterInfo::getCallPreservedMask(const MachineFunction &MF,
                                        CallingConv::ID CC) const {
  return CSR_RegMask;
}

const uint32_t*
HEIISRegisterInfo::getRTCallPreservedMask(CallingConv::ID CC) const {
  return RTCSR_RegMask;
}

BitVector HEIISRegisterInfo::getReservedRegs(const MachineFunction &MF) const {
...
void
HEIISRegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
  ...
  const HEIISSubtarget &Subtarget = MF.getSubtarget<HEIISSubtarget>();
  const HEIISFrameLowering *TFI = getFrameLowering(MF);
...
bool HEIISRegisterInfo::canRealignStack(const MachineFunction &MF) const {
  if (!TargetRegisterInfo::canRealignStack(MF))
    return false;

  // HEIIS always has a fixed frame pointer register, so don't need to
