//===-- HEIISFrameLowering.cpp - HEIIS Frame Information ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the HEIIS implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//

#include "HEIISFrameLowering.h"
...
DisableLeafProc("disable-sparc-leaf-proc",
                cl::init(false),
                cl::desc("Disable HEIIS leaf procedure optimization."),
                cl::Hidden);

HEIISFrameLowering::HEIISFrameLowering(const HEIISSubtarget &ST)
    : TargetFrameLowering(TargetFrameLowering::StackGrowsDown,
                          ST.is64Bit() ? 16 : 8, 0, ST.is64Bit() ? 16 : 8) {}

void HEIISFrameLowering::emitSPAdjustment(MachineFunction &MF,
...
void HEIISFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  const HEIISInstrInfo &TII =
      *static_cast<const HEIISInstrInfo *>(MF.getSubtarget().getInstrInfo());
  const HEIISRegisterInfo &RegInfo =
      *static_cast<const HEIISRegisterInfo *>(MF.getSubtarget().getRegisterInfo());
  ...
  NumBytes = MF.getSubtarget<HEIISSubtarget>().getAdjustedFrameSize(NumBytes);
...
MachineBasicBlock::iterator HEIISFrameLowering::
...
void HEIISFrameLowering::emitEpilogue(MachineFunction &MF,
                                  MachineBasicBlock &MBB) const {
  HEIISMachineFunctionInfo *FuncInfo = MF.getInfo<HEIISMachineFunctionInfo>();
...
bool HEIISFrameLowering::hasFP(const MachineFunction &MF) const {
...
int HEIISFrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI,
                                               unsigned &FrameReg) const {
  const HEIISSubtarget &Subtarget = MF.getSubtarget<HEIISSubtarget>();
  const MachineFrameInfo *MFI = MF.getFrameInfo();
  const HEIISRegisterInfo *RegInfo = Subtarget.getRegisterInfo();
  const HEIISMachineFunctionInfo *FuncInfo = MF.getInfo<HEIISMachineFunctionInfo>();
  ...
  // HEIIS uses FP-based references in general, even when "hasFP" is
...
bool HEIISFrameLowering::isLeafProc(MachineFunction &MF) const
{
...
void HEIISFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                              BitVector &SavedRegs,
                                              RegScavenger *RS) const {
  TargetFrameLowering::determineCalleeSaves(MF, SavedRegs, RS);
  if (!DisableLeafProc && isLeafProc(MF)) {
    HEIISMachineFunctionInfo *MFI = MF.getInfo<HEIISMachineFunctionInfo>();
